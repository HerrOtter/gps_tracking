                ...
                if len(track.segments) > 0:
                    for segment in track.segments:
                        for point in segment.points:
                            ...
                            if not is_valid:
                                is_valid_value = '0'
                                c.execute("INSERT INTO protocol (filename, track_id, is_valid) VALUES (?, ?, ?)", (filename, track_id, is_valid_value))
                                break
                            else:
                                is_valid_value = '1'
                                if tp_time is not None:
                                    ...
                                    tp_time_date = None
                                    tp_time_time = None
                                c.execute("INSERT INTO trackpoint (track_id, latitude, longitude, elevation, date, time, temperature, hr) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (track_id, lat, lon, ele, str(tp_time_date), str(tp_time_time), atemp, hr))
                                c.execute("UPDATE track SET date = ? WHERE id = ?", (tp_time_date, track_id))
                else:
                    is_valid_value = '0'
            c.execute("INSERT INTO protocol (filename, track_id, is_valid) VALUES (?, ?, ?)", (filename, track_id, is_valid_value))
        else:
                ...
                for waypoint in gpx.waypoints:
                    ...
                    if not is_valid:
                        ...
                        if tp_time is not None:
                            ...
                            tp_time_date = None
                            tp_time_time = None
                        c.execute("INSERT INTO trackpoint (track_id, latitude, longitude, elevation, date, time) VALUES (?, ?, ?, ?, ?, ?)", (track_id, lat, lon, ele, str(tp_time_date), str(tp_time_time)))
                        c.execute("UPDATE track SET date = ? WHERE id = ?", (tp_time_date, track_id))
...
def insert_vehicle_data(c, vehicle_license_plate):
    c.execute("INSERT INTO vehicle (license_plate) VALUES (?)", (vehicle_license_plate,))
    return c.lastrowid

def insert_track_data(c, filename, person_id, vehicle_id):
    c.execute("INSERT INTO track (filename, person_id, vehicle_id) VALUES (?, ?, ?)", (filename, person_id, vehicle_id))
    return c.lastrowid
...
def import_gpx_files():
    ...
    for filename in os.listdir('./data/gpx'):
        if filename.endswith('.gpx') and filename not in imported_files:
            ...
            track_id                = insert_track_data(c, track_filename, person_id, vehicle_id)
